...
*/
void mob_movement(int colunas, Map mapa[][colunas])
{
    for (int i = 0; i < 4; i++)
    {
...
*/
void mob_movement_far(int colunas, Map mapa[][colunas])
{
    for (int i = 0; i < 4; i++)
    {
...
void do_mob_apps(int colunas, Map mapa[][colunas])
{
    mob_view();
    close_to_player();
    mob_movement_far(colunas, mapa);
    mob_movement(colunas, mapa);
    mob_attacks();
}
/*
...
void do_player_punch(char c, int linhas, int colunas, Map mapa[][colunas])
...
void do_guns_aplications(char c, char player1_last_direction_moved, int linhas, int colunas, Map mapa[][colunas])
{
    ...
    do_player_punch(c, linhas, colunas, mapa);
    bullet_hit_mobs(linhas, colunas, mapa);
}
void do_guns_aplications_multi_player(char c, char player1_last_direction_moved, char player2_last_direction_moved, int linhas, int colunas, Map mapa[][colunas])
{
    change_player_weapon(c);
    bullet_position();
...
int player_relation_to_mob(int movement)
{
    switch (movement)
    {
    case 1:
...
int valid_player_movement(int movement, int colunas, Map mapa[][colunas])
{
    switch (movement)
    {
    case 1:
        if (mapa[player1.positionY - 1][player1.positionX].visible_piece != '#' &&
            ...
            player_relation_to_mob(movement) == 0)
            return 1;
        else
            return 0;
        break;
...
void do_update_map_single_player(char c, int colunas, Map mapa[][colunas], int linhas) // tem de receber o mapa desta forma pois é 2D
{
    ...
    if (c == 'w' && valid_player_movement(1, colunas, mapa) == 1)
    {
        player1_last_direction_moved = 'w'; // guardar ultima direçao em que se moveu
        player1.positionY--;
        player1_character = '^';
    }
    else if (c == 's' && valid_player_movement(2, colunas, mapa) == 1)
    {
        player1_last_direction_moved = 's';
        player1.positionY++;
        player1_character = 'v';
    }
    else if (c == 'a' && valid_player_movement(3, colunas, mapa) == 1)
    {
        player1_last_direction_moved = 'a';
        player1.positionX--;
        player1_character = '<';
    }
    else if (c == 'd' && valid_player_movement(4, colunas, mapa) == 1)
    {
...
void main_game_multi_player(char c, int linhas, int colunas, Map mapa[][colunas])
{
    while (game_over == 0)
    {
        if (player1.hp > 0 && player2.hp > 0)
        {
            ...
            do_guns_aplications_multi_player(c, player1_last_direction_moved, player2_last_direction_moved, linhas, colunas, mapa);
            do_add_player(2);
