...
void do_mob_application(int linhas, int colunas, Map mapa[][colunas], Mob mob)
{
    ...
    } while (count_walls_5x5(colunas, mapa, mob.positionY, mob.positionX) > 0 || mapa[mob.positionY][mob.positionX].visible_piece != ' ');
    ...
    if (is_mob_placed == 0) // caso nao haja mob ele insere-a e dá novas coordenadas à posição para checkar mais tarde
    {
        ...
        //tem de se ver
        top_left_corner_X = mob_positionX;
        top_left_corner_Y = mob_positionY;
        is_mob_placed = 1;
    }
}
/*
