...
int is_mob_placed = 0;
int is_in_mob_view = 0;
int damaging_player = 0;
int quadrado_down = 0;
int quadrado_right = 0;
int quadrado_up = 0;
int quadrado_left = 0;
 ...
 */
void mob_view(Mob mob)
{
   for (int coluna = 0; coluna <= 20; coluna++)
   {
      for (int linha = 0; linha <= 10; linha++)
      {
          if ((player1.positionX == (mob_positionX+coluna) && player1.positionY == (mob_positionY+linha)) || (player1.positionX == (mob_positionX-coluna) && player1.positionY == (mob_positionY+linha)) || (player1.positionX == (mob_positionX+coluna) && player1.positionY == (mob_positionY-linha)) || (player1.positionX == (mob_positionX-coluna) && player1.positionY == (mob_positionY-linha)))
          is_in_mob_view = 1;
      }
   }
}
...
*/
void mob_movement(int linhas, int colunas, Map mapa[][colunas], Mob mob)
{
float distancia = calc_dist(mob_positionY, player1.positionY, mob_positionX, player1.positionX);
float distanciaUmParaCima = calc_dist(mob_positionY - 1, player1.positionY, mob_positionX, player1.positionX);
float distanciaUmParaBaixo = calc_dist(mob_positionY + 1, player1.positionY, mob_positionX, player1.positionX);
float distanciaUmParaDireita = calc_dist(mob_positionY, player1.positionY, mob_positionX + 1, player1.positionX);
float distanciaUmParaEsquerda = calc_dist(mob_positionY, player1.positionY, mob_positionX - 1, player1.positionX);  

if((is_in_mob_view==1) && (damaging_player==0))
    {
      if (player1.positionX < mob_positionX) // Se o Player tiver à esquerda do Mob
      {
          if (player1.positionY > mob_positionY) // Se o Player tiver abaixo do mob
          {
              //Ve qual compensa mais: Andar para a Esquerda ou para Baixo
              if (distanciaUmParaEsquerda <= distanciaUmParaBaixo) 
                  {
                  if (!(mapa[mob_positionY][mob_positionX-1].visible_piece == '#')) mob_positionX -= 1; //Caso a esquerda nao  seja parede ele anda para a esquerda
                  else if (!(mapa[mob_positionY+1][mob_positionX].visible_piece == '#')) mob_positionY += 1;
                  }
              else 
                  {
                  if (!(mapa[mob_positionY+1][mob_positionX].visible_piece == '#')) mob_positionY += 1;  
                  else if (!(mapa[mob_positionY][mob_positionX-1].visible_piece == '#')) mob_positionX -= 1;   
                  }
          }
          else if (player1.positionY < mob_positionY) // Se o Player tiver acima do mob
          {
              //Ve qual compensa mais: Andar para a Esquerda ou para Cima
              if (distanciaUmParaEsquerda <= distanciaUmParaCima)
                  {
                  if (!(mapa[mob_positionY][mob_positionX-1].visible_piece == '#')) mob_positionX -= 1;
                  else if (!(mapa[mob_positionY-1][mob_positionX].visible_piece == '#')) mob_positionY -= 1;
                  }
              else 
                  {
      ...
      else if (player1.positionX > mob_positionX) // Se o Player tiver à direita do Mob
      {
          if (player1.positionY > mob_positionY) // Se o Player tiver abaixo do mob
          {
      ...
      {
      if (player1.positionY < mob_positionY) 
      {
        if (!(mapa[mob_positionY-1][mob_positionX].visible_piece == '#')) mob_positionY -= 1;
      }
      else if (player1.positionY > mob_positionY)
      {
        if (!(mapa[mob_positionY+1][mob_positionX].visible_piece == '#')) mob_positionY += 1;
      }
     }
...
*/
void mob_movement_far(int linhas, int colunas, Map mapa[][colunas], Mob mob)
{
if ((is_in_mob_view==0))
    {
      if (quadrado_down < 4)
      {
        if(mapa[mob_positionY+1][mob_positionX].visible_piece != '#')
        {
        mob_positionY += 1;
        quadrado_down++;
        }
      ...
      else if (quadrado_right < 5)
      {
        if(mapa[mob_positionY][mob_positionX+1].visible_piece != '#')
        {
        mob_positionX += 1;
        quadrado_right++;
        }
...
// A direção do ataque está relacionada com o ultimo movimento do jogador
{
    if (player1.gun == 1 && c ==' ')
    {
        if ((player1.positionX == mob_positionX && (player1.positionY-1) == mob_positionY) || ((player1.positionX-1)  == mob_positionX && (player1.positionY-1) == mob_positionY) || ((player1.positionX+1)  == mob_positionX && (player1.positionY-1) == mob_positionY ) || //Linha de Cima
        ((player1.positionX+1) == mob_positionX && player1.positionY == mob_positionY) || ((player1.positionX-1) == mob_positionX && player1.positionY == mob_positionY) || // Linha do Meio
        ((player1.positionX-1) == mob_positionX && (player1.positionY+1) == mob_positionY) || (player1.positionX == mob_positionX && (player1.positionY+1) == mob_positionY) || ((player1.positionX+1) == mob_positionX && (player1.positionY+1) == mob_positionY)) //Linha de Baixo
        {
        mob.hp -= 25;
        if (mob.hp <= 0)
        {
...
void do_structure_aplications(int colunas, Map mapa[][colunas])
{
    ...
    if (player1.positionX-1 == mob_positionX && player1.positionY == mob_positionY)
    {
     player1.hp -= 2;   
    }
}
...
void print_footer(int linhas, int colunas, int n)
{
    ...
    if (player1.hp == 100)
    {
        ...
        if (player1.score >= 0 && player1.score < 10)
        {
            ...
            mvprintw(linhas - 1, colunas - 54, "Ammo: %d", player1.ammo);
        }
        else if (player1.score >= 10 && player1.score < 100)
        {
            ...
            mvprintw(linhas - 1, colunas - 55, "Ammo: %d", player1.ammo);
        }
        else if (player1.score >= 100)
        {
            ...
            mvprintw(linhas - 1, colunas - 56, "Ammo: %d", player1.ammo);
        }
    }
...
void player_position(int linhas, int colunas, Map mapa[][colunas])
{
    ...
    for (int i = 2; i < colunas - 2; i++)
    {
        ...
        {
            if (count_walls_3x3(colunas, mapa, j, i) == 0) // colocar o jogador num quadrado livre 3x3
            {
                player1.positionX = i;
                player1.positionY = j;
                // quebrar o ciclo
...
void main_game(char c, int linhas, int colunas, Map mapa[][colunas])
{
    while (game_over == 0)
    {
        if (player1.hp > 0)
        {
            ...
            mob_view(mob);
            close_to_player(mob);
            mob_movement_far(linhas, colunas, mapa, mob);
            mob_movement(linhas, colunas, mapa, mob);
            mob_attacks(mob);
            ...
            attron(COLOR_PAIR(5));
            mvaddch(player1.positionY, player1.positionX, player_character);
